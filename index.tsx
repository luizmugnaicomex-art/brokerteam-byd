...
const formatCurrency = (value: number | undefined | null, currency?: string | null) => {
    if (value === undefined || value === null || isNaN(value)) {
        return '-';
    }
    // FIX: Default to 'BRL' if the provided currency is null, undefined, or an empty string to prevent runtime errors.
...
// FIX: Removed duplicate 'onImport' prop to resolve a duplicate identifier error.
...
// --- App Component ---
const App = () => {
  // Check for placeholder config first
    ...
    return (
        <div className="connection-error-screen">
            <div className="login-box animate-scale-in">
                 ...
                 <p className="login-subtitle" style={{textAlign: 'left', maxWidth: 'none'}}>The application cannot connect to the database. This is usually because the Firebase configuration is incomplete.</p>
                 <div className="config-instructions">
                    <p><strong>Next Steps:</strong></p>
                    <ol>
                        <li>Open the <code>firebase.tsx</code> file in your project.</li>
                        <li>Replace the placeholder values (like <code>"YOUR_API_KEY"</code>) with your actual credentials from the Firebase Console.</li>
  ...
  // --- Firebase Data & Auth Listeners ---
    useEffect(() => {
        ...
        const unsubscribeAuth = auth.onAuthStateChanged(async (userAuth: any) => {
            if (userAuth) {
                ...
                if (userDoc.exists) {
                    ...
                    // Self-healing: User exists in Auth, but not Firestore. Create a default profile.
                    console.warn(`User document not found in Firestore for UID: ${userAuth.uid}. Creating a default profile.`);
                    const defaultUserData: Omit<User, 'id' | 'password'> = {
                        name: userAuth.displayName || userAuth.email.split('@')[0],
                        username: userAuth.email,
                        role: 'Broker', // Assign a default, non-privileged role
                    };
                    ...
                    try {
                        ...
                        console.error("Failed to create default user profile for orphaned auth user:", error);
                        setLoginError("Your user profile is missing and could not be repaired. Please contact an administrator.");
                        auth.signOut();
                    }
                }
            } else {
        ...
        // Firestore listeners
        const unsubscribeShipments = firestore.collection('shipments').onSnapshot(snapshot => {
        ...
        const unsubscribe5W2H = firestore.collection('fiveW2H').onSnapshot(snapshot => {
            const data = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })) as FiveW2H[];
            setFiveW2HData(data);
        });

        const unsubscribeRates = firestore.collection('config').doc('exchangeRates').onSnapshot(doc => {
            if (doc.exists) {
                setExchangeRates(doc.data() as ExchangeRates);
            }
        });
        
        // TODO: Add listeners for claims and tasks when their collections are created in Firestore
  ...
  const updateShipment = (updatedShipment: Shipment) => {
    ...
    const handleSaveUser = async (user: User) => {
        let newUserAuth: any = null; // To hold the created auth user for potential rollback
        try {
            if (user.id) { // Update existing user
                ...
                if (newUserAuth) {
                    // Now, create the Firestore document. If this fails, the catch block will run.
                    await firestore.collection('users').doc(newUserAuth.uid).set({
                        name: user.name,
            ...
            // If we created an auth user but the process failed afterwards (e.g., Firestore write failed)
            if (newUserAuth) {
                console.error("Firestore user document creation failed after auth user was created. Attempting to roll back auth user creation...");
                await newUserAuth.delete().catch((deleteError: any) => {
                    console.error(`CRITICAL: Failed to delete orphaned auth user with UID ${newUserAuth.uid}. Please delete it manually from the Firebase console.`, deleteError);
                    // Inform the admin user that a manual cleanup is needed.
                    alert(`CRITICAL ERROR: User creation failed and the rollback was unsuccessful. A user with email ${user.username} exists in the authentication system but not in the database. Please delete this user manually from the Firebase Authentication console.`);
                });
            }
    ...
    try {
        ...
        for (const row of jsonData) {
            ...
            for (const rawHeader in row) {
                ...
                if (key) {
                    (newShipment as any)[key] = value !== null && value !== '' ? value : null;
                }
            }
            ...
            numberFields.forEach(field => {
                if (newShipment[field] !== undefined && newShipment[field] !== null) {
                    ...
                    (newShipment as any)[field] = isNaN(numVal) ? null : numVal;
                }
            });
